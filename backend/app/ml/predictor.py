...
class RacePredictor:
    ...
    def __init__(self):
        if not self.initialized:
            ...
            # Temporary encoder maps until we retrain and persist real encoders.
            # The model was trained on 2024 codes, so 2025-only drivers are approximate.
    ...
    def predict(self, state: RaceState, scenario_config=None, prediction_tier: str = "full") -> Dict:
        """Run the full race prediction pipeline.

        Flow: model priors -> log-odds adjustments -> MC sampling.
        We keep adjustments in logit space and convert back with sigmoid.
        """
        ...
        # Strategy optimization is expensive; keep preview mode lightweight.
        strategy_cars_limit = 2 if tier == "preview" else 3

        data = []
        total_laps = state.meta.laps_total
        
        # Stage 0: assimilate live state and compute pace corrections.
        ...
        # Stage 1: calibrated model priors.
        ...
        # Stage 2: apply scenario and RL modifiers in logit space.
        # Additive logit updates are equivalent to multiplying odds ratios.
        
        win_prob_dict = {}
        podium_prob_dict = {}
        causal_factors = {}
        rl_signals_dict = {}  # Passed into Monte Carlo for per-driver variance.
        ...
        for i, car in enumerate(state.cars):
            ...
            raw_p_win = p_win  # Kept for explanation features.
            ...
            # Chaos pulls extreme probabilities toward the middle.
            ...
            if tire_deg_multiplier > 1.0 and tire_age > 8:
                ...
                tire_term = np.log(deg_penalty)  # Negative effect.
            elif tire_deg_multiplier < 1.0:
                deg_boost = 1.0 + (1.0 - tire_deg_multiplier) * 0.1
                tire_term = np.log(deg_boost)  # Positive effect.
            ...
            # Qualifying override term, scaled by current position.
            quali_term = 0.0
            if qualifying_delta != 0.0:
                position_factor = max(0.05, 1.0 - (car.timing.position - 1) / 20.0)
                quali_term = qualifying_delta * position_factor * 0.5  # Dampened.
            ...
            if da_factor > 0.05:
                dirty_air_term = np.log(max(0.5, 1.0 - da_factor * 0.15))  # Capped penalty.
            
            # Track evolution term.
            track_grip_term = 0.0
            if hasattr(state.track, 'track_evolution') and state.track.track_evolution:
                ...
                if sim_grip > 1.02:
                    track_grip_term = np.log(1.0 + (sim_grip - 1.0) * 0.05)
            ...
            if scenario_config:
                ...
                if driver_cfg:
                    ...
                    if champ_pos > 0:
                        ...
                        if champ_pos <= 3:
                            if champ_pos == 1:
                                championship_term = -0.05 * (2.0 - pressure_handling)
                            else:
                                championship_term = 0.08 * pressure_handling
            
            # Compose posterior logits.
            ...
            # Build a lightweight explanation list for the UI.
            factors = []
            if raw_p_win > 0.10:
            ...
            # Interaction and momentum factors.
            if da_factor > 0.2:
        ...
        # Blend with uniform to avoid overconfidence.
        ...
        # Cap single-driver dominance before MC.
        MAX_WIN_CAP = 0.55
        for cap_pass in range(3):
            for idx in range(n_drivers):
                if final_win[idx] > MAX_WIN_CAP:
                    excess = final_win[idx] - MAX_WIN_CAP
                    final_win[idx] = MAX_WIN_CAP
                    # Redistribute excess across the rest of the field.
        ...
        # Optional field compression from scenario config.
        if field_compression != 1.0:
            mean_win = final_win.mean()
            # Interpolate toward/away from the mean.
        ...
        # Podium probabilities use the same smoothing approach.
        ...
        # Stage 4: run Monte Carlo with RL-informed variance.
        # Build championship data for rivalry effects.
        ...
        for idx, car in enumerate(sorted_cars):
            ...
            # Positive value means follower has fresher tires than the car ahead.
            if idx > 0:
                leader_car = sorted_cars[idx - 1]
                tire_deltas[d] = leader_car.telemetry.tire_state.wear - car.telemetry.tire_state.wear
            
            # Driver personality (scenario config if present).
        ...
        # Stage 5: strategy optimization for top contenders.
        strategy_recommendations = {}
        # Limit optimization count for runtime.
